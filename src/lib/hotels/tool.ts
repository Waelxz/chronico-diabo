...
import { fetchHotelsNear } from '@/lib/overpass-hotels';
...
export async function findHotelsForChat({ near }: { near: string }) {
  try {
    const location = resolveLocation(near);
    const pois = await fetchHotelsNear({
      lat: location.lat,
      lon: location.lon,
      radius: 1_500,
      timeoutMs: 8_000,
    ...
    if (pois.length === 0) {
      return {
        ...
        hotels: [],
        note: "Aucun hôtel trouvé via OpenStreetMap dans cette zone. La couverture peut être incomplète.",
      };
    }

    const ranked = await rankHotels({
      hotels: pois,
    ...
    return {
      location: location.label,
      locationApproximate: location.approximate,
      hotels: ranked.slice(0, 3).map((hotel) => ({
        name: hotel.name,
    ...
    console.error('[findHotelsForChat] error:', err);
    return {
      location: near,
      locationApproximate: true,
      hotels: [],
      note: `Erreur lors de la récupération des hôtels: ${err instanceof Error ? err.message : 'erreur inconnue'}. Suggère à l'utilisateur d'essayer la page /hotels directement.`,
    };
  }
}
